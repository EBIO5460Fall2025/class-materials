...
plot(x, pnorm(x), type="l", col="goldenrod2", main="Cumulative distribution function")
axis(2, at=seq(0, 1, by=0.1), labels=FALSE, tcl=-0.25)
```

![](15_3_quantiles-qqplots_files/figure-gfm/unnamed-chunk-1-1.png)<!-- -->

Quantiles are the inverse of this function: the value of x for which the
cumulative probability is p. For example, what value of x is 80% of the
values of x expected to be less than? The quantile is given by
`qnorm()`.
...
plot(x, pnorm(x), type="l", col="goldenrod2", main="Cumulative distribution function")
axis(2, at=seq(0, 1, by=0.1), labels=FALSE, tcl=-0.25)
segments(-4, p, qnorm(p), p, col="steelblue2")
segments(qnorm(p), -0.1, qnorm(p), p, col="steelblue2")
text(qnorm(p), 0, labels=signif(qnorm(p), 4), pos=4)
```

![](15_3_quantiles-qqplots_files/figure-gfm/unnamed-chunk-3-1.png)<!-- -->
...
The basic idea of a QQ (quantile-quantile) plot is to plot the empirical
quantiles (i.e. from the data) against the theoretical quantiles of the
...
``` r
plot(q, p, type="l", col="goldenrod2", main="Empirical cdf (golden line)")
axis(2, at=seq(0, 1, by=0.1), labels=FALSE, tcl=-0.25)
for ( i in 1:n ) {
    segments(-4, p[i], q[i], p[i], col="steelblue2")
    segments(q[i], -0.1, q[i], p[i], col="steelblue2")    
}
```

![](15_3_quantiles-qqplots_files/figure-gfm/unnamed-chunk-10-1.png)<!-- -->
...
``` r
plot(x, pnorm(x), type="l", col="goldenrod2", ylab="p",
     main="Theoretical cdf (golden curve)")
axis(2, at=seq(0, 1, by=0.1), labels=FALSE, tcl=-0.25)
for ( i in 1:n ) {
    segments(-4, p[i], qth[i], p[i], col="steelblue2")
    segments(qth[i], -0.1, qth[i], p[i], col="steelblue2")    
}
```

![](15_3_quantiles-qqplots_files/figure-gfm/unnamed-chunk-12-1.png)<!-- -->
...
![](15_3_quantiles-qqplots_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->
...
![](15_3_quantiles-qqplots_files/figure-gfm/unnamed-chunk-14-1.png)<!-- -->
