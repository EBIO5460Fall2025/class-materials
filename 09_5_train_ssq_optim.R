...
# In this example, there is no strong advantage to separating out the model from
# the SSQ function but this program structure generalizes to more complex models
...
#----lin_skel()-----------------------
# A function for the deterministic skeleton of the model algorithm, in this case
# a linear model
...
ssq_lin_skel <- function(p, y, x) {
    ...
    #is self documenting
    y_pred <- lin_skel(b_0=p[1], b_1=p[2], x) #predicted y
...
fit <- optim(par, ssq_lin_skel, y=linear_data$y, x=linear_data$x)
fit
...
# Calculate the predictions from the trained model
# Also called "fitted values".

y_pred <- lin_skel(b_0=fit$par[1], b_1=fit$par[2], linear_data$x)
