...
# models fitted by likelihood methods. This is script 2, which includes case
# deletion diagnostics that are useful for diagnosing influence and outliers in
# the general likelihood case.

# The model for the data generating process is a nonlinear stochastic model for
# the population dynamics of a species of Tribolium beetle. Data are from
# Melbourne & Hastings (2008) Nature 454: 100-103.

# Brett Melbourne 18 Oct 2018

# This model is set up for maximum likelihood estimation via `optim()`. The
# model has three parameters to estimate: R, alpha, dp. The deterministic
# skeleton is an ecological function for population growth as a function of
...
# Model trained via maximum likelihood on the full dataset.
...
# Case deletion fits. Leave one out (LOO) algorithm.
