...
In this example, the task is to bootstrap a *p*-value for the slope $\beta_1$ of a linear model. While the example uses the simple linear model, this general algorithm can be applied to any kind of model, and a p-value about any hypothesis.
...
Record estimated slope, $\hat\beta_1$, and estimated standard deviation of the DGP, $\hat\sigma_e$
...
Train null model ($y = \beta_0 + e$). `y ~ 1` specifies the intercept only model (see `?formula`).

```{r}
fit_m0 <- lm(y ~ 1)
```

Record estimated intercept for the null model, $\hat\beta_{0,\mathrm{null}}$
...
Calculate two-tailed *p*-value, which is the notion that the magnitude of the observed $\hat\beta_1$ is at issue and not its specific sign, so we need to consider magnitudes of $\beta_1$ of either sign in the sampling distribution.
...
As usual, the histogram shows the bootstrapped sampling distribution. I've added a rug plot, which puts a tickmark for each bootstrap replicate, useful here to visualize the tails. I've also added a solid gold line to show the observed (fitted) value for $\hat\beta_1$ and a dashed gold line to show the value of $-\hat\beta_1$ indicating a slope of the same magnitude but in the negative direction. Bootstrap samples outside these lines are the values "as large or larger than" the observed magnitude of $\hat\beta_1$. I've also overlaid a Normal distribution on the histogram. We see that the sampling distribution for $\beta_1$ tends Normal, as expected from theory based on the assumptions of the null model.

Compare our version of the bootstrap *p*-value to the classical *p*-value. It's similar but slightly smaller than the classical *p*-value for these data. The appropriate *p*-value is on line `x`.

```{r}
summary(fit_m1)$coefficients
```

Here we've used $\beta_1$ straight up as the test statistic in the bootstrap. If we instead use the *t* statistic (i.e. divide $\hat\beta_1$ by its estimated standard error), which matches the setup of the classical test, we find that the bootstrapped *p*-value is almost identical to the classical test. These are both valid and correct *p*-values, just for different test statistics.
